...
def inicjalizacjaPlanszy(root, gorny_panel, ikonki):
	...
	for i in range(N):
		for j in range(M):
			...
			przyciski[i*M + j].bind('<Button-1>', lambda event, p = przyciski[i*M + j]: lewyKlik(przyciski, p, gorny_panel, tablica_gry, ikonki))
			przyciski[i*M + j].bind('<Button-3>', lambda event, p = przyciski[i*M + j] : prawyKlik(przyciski, p, gorny_panel, tablica_gry, ikonki))
			przyciski[i*M + j].bind('<ButtonRelease>', lambda event : zwyklaBuzka(gorny_panel[1]))
...
def wczytajIkonki():
	...
	ikonki['buzki'] = [tk.PhotoImage(file = 'buzka' + str(i) + '.png') for i in range(1,5)]
	ikonki['flaga'] = tk.PhotoImage(file = 'flaga.png')
	ikonki['miny'] = [tk.PhotoImage(file = 'mina.png'), tk.PhotoImage(file = 'pierwsza.png')]
...
def przegranaGra(przyciski, tablica_gry, ikonki):
	...
	for i in range(N):
		for j in range(M):
			if isinstance(przyciski[i*M + j], tk.Button) and przyciski[i*M + j]['state'] != 'disabled':
...
def aktualizujPrzycisk(przyciski, przycisk, indeks, pole, tablica_gry, ikonki):
	...
	if pole != 'x':
		if pole != 0:
			...
			for x, y in sasiedzi:
				if isinstance(przyciski[y*M + x], tk.Button) and przyciski[y*M + x]['state'] != 'disabled':
					aktualizujPrzycisk(przyciski, przycisk, y*M + x, tablica_gry[y][x], tablica_gry, ikonki)
...
if __name__ == '__main__':
	...
	gorny_panel[1].bind('<Button-1>', lambda event: resetujGre(root, gorny_panel, ikonki))
